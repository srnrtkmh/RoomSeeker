...
PROGMEM const uint8_t ENC_RESO = 22;
...
int16_t Fd_p_x100[4] = {0, 0, 0, 0};  // Dynamic friction compensation parameter for negative direction
int16_t Fd_n_x100[4] = {0, 0, 0, 0};  // Dynamic friction compensation parameter for negative direction
    ...
    if (str[0] == 'w' && str[1] == 'r' && str[2] == 'k') {
      if (str[26] == 'e' && str[27] == 'n' && str[28] == 'd') {
        if (str[3] == ',' && str[7] == ',' && str[13] == ',' && str[19] == ',' && str[25] == ',') {
          if (str[8] == '+')       work_vel_cmd_x10[0] =   ((long)str[ 9] - 48) * 1000 + ((long)str[10] - 48) * 100 + ((long)str[11] - 48) * 10 + ((long)str[12] - 48);
          else if (str[8] == '-')  work_vel_cmd_x10[0] = -(((long)str[ 9] - 48) * 1000 + ((long)str[10] - 48) * 100 + ((long)str[11] - 48) * 10 + ((long)str[12] - 48));
          if (str[14] == '+')      work_vel_cmd_x10[1] =   ((long)str[15] - 48) * 1000 + ((long)str[16] - 48) * 100 + ((long)str[17] - 48) * 10 + ((long)str[18] - 48);
          else if (str[14] == '-') work_vel_cmd_x10[1] = -(((long)str[15] - 48) * 1000 + ((long)str[16] - 48) * 100 + ((long)str[17] - 48) * 10 + ((long)str[18] - 48));
          if (str[20] == '+')      work_vel_cmd_x10[2] =   ((long)str[21] - 48) * 1000 + ((long)str[22] - 48) * 100 + ((long)str[23] - 48) * 10 + ((long)str[24] - 48);
          else if (str[20] == '-') work_vel_cmd_x10[2] = -(((long)str[21] - 48) * 1000 + ((long)str[22] - 48) * 100 + ((long)str[23] - 48) * 10 + ((long)str[24] - 48));
          start_bit = 1;
          workspace_ctrl = 1;
          ps2_ctrl = 0;
        }
...
void print_label(void) {
  ...
  // For debug
  //  Serial.print(F("n, cnt_now[0], cnt_now[1], cnt_now[2], cnt_now[3], diff_cnt[0], diff_cnt[1], diff_cnt[2], diff_cnt[3], omega_res_x10[0], omega_res_x10[1], omega_res_x10[2], omega_res_x10[3], omega_res_x10_lpf[0], omega_res_x10_lpf[1], omega_res_x10_lpf[2], omega_res_x10_lpf[3], omega_cmd_x10[0], omega_cmd_x10[1], omega_cmd_x10[2], omega_cmd_x10[3], vout[0], vout[1], vout[2], vout[3]\n"));
}
...
void inv_kinematics(void) {
  omega_cmd_x10[0] = (work_vel_cmd_x10[0] + work_vel_cmd_x10[1] + (W + L) / 2 * work_vel_cmd_x10[2] / 180 * 314 / 100 / 10) / R * 18000 / 314;
  omega_cmd_x10[1] = (work_vel_cmd_x10[0] - work_vel_cmd_x10[1] - (W + L) / 2 * work_vel_cmd_x10[2] / 180 * 314 / 100 / 10) / R * 18000 / 314;
  omega_cmd_x10[2] = (work_vel_cmd_x10[0] - work_vel_cmd_x10[1] + (W + L) / 2 * work_vel_cmd_x10[2] / 180 * 314 / 100 / 10) / R * 18000 / 314;
  omega_cmd_x10[3] = (work_vel_cmd_x10[0] + work_vel_cmd_x10[1] - (W + L) / 2 * work_vel_cmd_x10[2] / 180 * 314 / 100 / 10) / R * 18000 / 314;
}
...
void setup() {
  ...
  if (ps2_error == 0) {
    ...
    if (pressures) Serial.println(F("true"));
...
void loop() {
  ...
  if (ps2_type != 2) {    //DualShock Controller
    ...
    if (ps2x.Button(PSB_L1)) {         // Turn left if the L1 is pressed
      if (ps2x.Button(PSB_CROSS)) {
        if (ps2x.Button(PSB_SQUARE)) work_vel_cmd_x10[2] = 6000;
        else                         work_vel_cmd_x10[2] = 4000;
      } else                         work_vel_cmd_x10[2] = 2000;
      start_bit = 1;
      workspace_ctrl = 1;
      ps2_ctrl = 1;
    }
