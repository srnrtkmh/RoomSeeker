...
if __name__ == '__main__':
  ...
  # キーボード入力用スレッド作成
  t1 = threading.Thread(target=key_input)
  t1.setDaemon(True)
  t1.start()
  ...
  log_file.write(str(currentDate) + " : Started." + "\n")
  
  # 実行開始直後使用ファイルを開き、所有者を変更
  strCsvName = unicode("./" + preffix + currentDate.strftime('%Y_%m_%d_%H_%M_%S') + suffix + ".csv", encoding='shift-jis')  # ファイル名の設定
  ...
  try:
    while True:
      ...
      if ctrl_cnt >= 10:
        ctrl_cnt = 0
        if ctrl_state == 0:
          con.write("vel,000,1000,1000,1000,1000,end")
          ctrl_state = 1
        elif ctrl_state == 1:
          con.write("vel,000,2500,2500,2500,2500,end")
          ctrl_state = 2
        elif ctrl_state == 2:
          con.write("vel,000,5000,5000,5000,5000,end")
          ctrl_state = 3
        elif ctrl_state == 3:
          con.write("vel,000,2500,2500,2500,2500,end")
          ctrl_state = 4
          ...
          con.write("vel,000,0000,0000,0000,0000,end")
          ctrl_state = 0
      ctrl_cnt = ctrl_cnt + 1
