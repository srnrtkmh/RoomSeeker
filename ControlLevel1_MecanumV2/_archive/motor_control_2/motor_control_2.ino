...
long Kp[4] = {1, 1, 1, 1};                      // P gain for PI control
long Ki[4] = {1, 1, 1, 1};                      // I gain for PI control
...
uint32_t int_enc = 0, int_enc2 = 0;
uint32_t int_ctr = 0, int_ctr2 = 0;
uint32_t int_out = 0;
uint32_t int_prt = 0;
uint32_t tmp_micros = 0, tmp_micros2 = 0;
uint32_t tmp_cnt = 0;
...
ISR(PCINT2_vect) {
  ...
  lll = micros();
  int_enc = lll - kkk;
  if (tmp_micros > tmp_micros2) int_enc2 = lll - tmp_micros;
  else                         int_enc2 = lll - tmp_micros2;
  tmp_cnt++;
}
...
#if defined(DEBUG)
  sprintf(str, "%5d,%5d,%5d,%6d,", (int)int_prt, (int)int_enc, (int)int_enc2, (int)tmp_cnt);
  Serial.print(str);
  sprintf(str, "%5d,%5d,%5d,", (int)int_ctr, (int)int_ctr2, (int)int_out);
  Serial.print(str);
#endif
  sprintf(str, "%+4d", (int)n);
  Serial.print(str);
  for (i = 0; i < 4; i++) {
    sprintf(str, ",%+6d,%+6d,%+5d", (int)cnt_now[i], (int)omega_res_x10[i], (int)vout[i]);
    Serial.print(str);
#if defined(DEBUG) || defined(LONG_MSG)
    sprintf(str, ",%+5d,%5d", (int)omega_res_x10_lpf[i], (int)omega_cmd_x10[i]);
    Serial.print(str);
    sprintf(str, ",%+5d,%5d", (int)vout_ff[i], (int)vout_fb[i]);
#endif
  }
  Serial.print("\n");
  lll = micros();
...
void flash() {
  ...
  if (start_bit == 1) {
    ...
    for (i = 0; i < 4; i++) {
      omega_res_x10[i] = (diff_cnt[i] * 360) * _dt_x10 / 1320;
    ...
    for (i = 0; i < 4; i++) {
      ...
      // Feedback control
      vout_fb[i] = Kp[i] * e_omega[i] / 100 + Ki[i] * int_e_omega[i] / 10000;
